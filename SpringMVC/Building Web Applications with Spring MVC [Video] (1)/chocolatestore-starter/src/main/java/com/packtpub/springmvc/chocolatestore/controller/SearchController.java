...
public class SearchController {
	...
	public String processSearch(@Valid Product product, BindingResult bindingResult, Model model) {
		...
		/*
		 * this can be removed as we are initializing & calling custom validator through @Init below
		 * 
		 *  todo that, we declare our validator as bean & autowire its member var & set to webdatabinder
		 *  
		 *  this way custom validator gets called for every method in the controller, also we need to delcae @Valid
		 *  
		 *  at the Product		 *  
		 * 
		 * 	new ProductSearchValidator().validate(product, bindingResult); 
		 *
		 */
    ...
    private void initBinder(WebDataBinder binder) {  
        ...
        binder.setAllowedFields("name", "category"); //we can allow few fields of our choice
    }
